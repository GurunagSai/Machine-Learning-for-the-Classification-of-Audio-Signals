...
def get_labels(path_to_dir,csv_file):
...
"""def generate_spectrograms(dir_path, csv_file, extracted_folder):
    df = pd.read_csv(os.path.join(dir_path, csv_file))
    audio_extracted = np.array(df['file name'])

    for i in range(len(audio_extracted)):
        audio_file_path = os.path.join(dir_path, extracted_folder, str(audio_extracted[i].replace(".wav", ".npy")))
        audio_file_data = np.load(audio_file_path)
        spectrogram = librosa.feature.melspectrogram(y=audio_file_data, sr=44100)
        spectrogram_db = librosa.power_to_db(spectrogram, ref=np.max)
         plt.figure(figsize=(10, 4))
        librosa.display.specshow(spectrogram_db, x_axis='time', y_axis='mel', sr=44100, fmax=8000)
        plt.colorbar(format='%+2.0f dB')
        plt.title(f'Spectrogram for {audio_extracted[i]}')
        plt.savefig(os.path.join(dir_path, 'spectrograms', f'spectrogram_{audio_extracted[i].replace(".wav", ".png")}'))
        plt.close() """
...
def get_spectrograms(path_to_dir, train_csv , train_extracted, test_csv ,test_extracted, target_shape=(128, 128)):
  ...
  for i in range(len(audio_extracted)):
        ...
        mel_spec = librosa.power_to_db(librosa.feature.melspectrogram(y=audio_file_data[:650000], n_mels=target_shape[0], hop_length=2048), ref=np.max)
...
#-------------MAIN-------------# 
path_to_dir = 'E:/Individual Project/AudioClassification-main/AudioClassification-main'
...
X_train,Y_train,X_val,Y_val, X_test = get_spectrograms(path_to_dir,'TrainDataSetCSV.csv','train_extracted', 'TestDataSetCSV.csv','test_extracted', target_shape=(128, 128))
